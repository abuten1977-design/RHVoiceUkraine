...
@interface FixedRHVoiceWrapper : NSObject
...
@implementation FixedRHVoiceWrapper

static RHVoice_tts_engine engine = NULL;

+ (void)initializeRHVoice {
    NSLog(@"🔧 Инициализация RHVoice с абсолютными путями...");
    ...
    // АБСОЛЮТНЫЙ путь к данным RHVoice
    const char* data_path = "";
    NSLog(@"📁 Используем путь к данным: %s", data_path);
    ...
    if (engine) {
        ...
        NSLog(@"❌ Проверьте путь к данным: %s", data_path);
    }
}
