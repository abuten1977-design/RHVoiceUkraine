...
Голоси **Ben (bdl)**, **Clara (clb)**, **Sarah (slt)**, **Raj (ksp)** не входять
...
1. Додати до архівів завантажуваних англійських голосів файл ліцензії CMU і
   позначку про те, що моделі є зміненими похідними.
2. Отримати дозвіл команди «Синтезатор української мови» щодо голосів
   **Marianna і Volodymyr** (CC BY-ND 4.0) — згоду ліцензіара за розділом
   2(a)(5)(B) на застосування платформою технічних засобів захисту. Для
   **Anatol і Natalia** (LGPL-2.1) дозвіл не потрібен: §6 дозволяє
   розповсюдження «under terms of your choice», якщо умови допускають
   модифікацію для власного вжитку та зворотну розробку для налагодження — саме
   це дозволяє виняток для open-source у стандартному EULA Apple.

Політику приватності опубліковано:
<https://abuten1977-design.github.io/RHVoiceUkraine/privacy.html>.

**Власний ліцензійний договір (Custom EULA) НЕ подається.** Стандартний
Licensed Application EULA Apple містить виняток для компонентів з відкритим
кодом («except … to the extent as may be permitted by the licensing terms
governing use of any open-sourced components»), а App Store exception автора
рушія знімає конфліктні умови GPL-3 щодо розповсюдження через App Store. Так
само чинять зіставні застосунки (Nextcloud iOS — GPL-3 з exception і
стандартний EULA). Файл `EULA.md` лишається документом проєкту для
користувачів, а не угодою, що подається в App Store Connect.
