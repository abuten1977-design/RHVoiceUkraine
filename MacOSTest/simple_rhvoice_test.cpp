...
int main() {
    std::cout << "🚀 Тест RHVoice на macOS" << std::endl;
    ...
    // Путь к данным RHVoice (абсолютный)
    ...
    // Создаем TTS engine
    std::cout << "🔧 Создаем RHVoice TTS engine..." << std::endl;
    RHVoice_tts_engine engine = RHVoice_new_tts_engine(&init_params);
    
    if (engine) {
        std::cout << "✅ RHVoice TTS engine создан успешно!" << std::endl;
        ...
        // Тест синтеза
        std::cout << "🗣️ Начинаем синтез..." << std::endl;
        audio_buffer.clear();
        
        // Параметры синтеза
        RHVoice_synth_params synth_params = {0};
        synth_params.voice_profile = "natalia";
        
        // Создаем сообщение
        const char* text = "Привіт! Це тест RHVoice на macOS. Слава Україні!";
        ...
        if (message) {
            std::cout << "📝 Сообщение создано, выполняем синтез..." << std::endl;
            
            int result = RHVoice_speak(message);
            RHVoice_delete_message(message);
            
            if (result && !audio_buffer.empty()) {
                std::cout << "✅ Синтез успешен: " << audio_buffer.size() << " samples" << std::endl;
                
                // Сохраняем WAV файл
                save_wav("macos_rhvoice_test.wav", audio_buffer, current_sample_rate);
                std::cout << "🎉 УСПЕХ! Создан настоящий украинский голос на macOS!" << std::endl;
            } else {
                std::cout << "❌ Синтез не удался" << std::endl;
            }
        } else {
            std::cout << "❌ Не удалось создать сообщение" << std::endl;
        }
        
        RHVoice_delete_tts_engine(engine);
    } else {
        std::cout << "❌ Не удалось создать RHVoice TTS engine" << std::endl;
        return 1;
    }
